  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await adminService.createGalleryItem({
        folder_path: formData.folder_path as 'assets/gallery' | 'assets/transformations',
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <Card className="p-6">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-garage-muted">No gallery items registered.</div>
          ) : (
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <div className="p-4 flex-grow flex flex-col justify-between">
                    <div>
                      <p className="text-xs font-bold text-garage-white truncate">
                        {item.file_name}
                      </p>
                      {item.caption && (
                        <p className="text-[11px] text-garage-muted mt-1">{item.caption}</p>
                      )}
                    </div>

                    <div className="mt-4 pt-3 border-t border-garage-mid/50 flex items-center justify-between">
                      <span className="text-[10px] text-garage-muted">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <SelectField
              ...
              options={[
                ...
                {
                  value: 'assets/transformations',
                  label: 'Transformations (assets/transformations)',
                },
              ]}
            />
