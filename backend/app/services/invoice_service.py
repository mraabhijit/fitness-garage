...
def generate_invoice_pdf(
    ...
    plan_str = f"{plan_tier.upper() if plan_tier else 'STANDARD'} — {plan_duration.capitalize() if plan_duration else 'MEMBERSHIP'}"
    details_data = [
        [
        ...
        [
            Paragraph(f"Member: {member_name}<br/>Phone: {member_phone or 'N/A'}<br/>Email: {member_email or 'N/A'}", body_style),
            Paragraph(f"Invoice ID: #{str(payment_id)[:8]}<br/>Date: {payment_date.strftime('%d %b %Y')}<br/>Method: {payment_method.upper()}", body_style),
        ],
    ]
    ...
    items_data = [
        [
            Paragraph("<b>Description</b>", ParagraphStyle("Hdr", fontName="Helvetica-Bold", textColor=colors.white)),
            Paragraph("<b>Duration</b>", ParagraphStyle("Hdr", fontName="Helvetica-Bold", textColor=colors.white)),
            Paragraph("<b>Amount (INR)</b>", ParagraphStyle("Hdr", fontName="Helvetica-Bold", textColor=colors.white, alignment=2)),
        ],
        [
        ...
        [
            ...
            "",
            Paragraph(f"<b>₹{amount:,.2f}</b>", ParagraphStyle("Tot", fontName="Helvetica-Bold", alignment=2)),
        ],
    ]
    ...
    # Footer Notes
    story.append(Paragraph("Thank you for training with Fitness Garage!", ParagraphStyle("Foot", fontName="Helvetica-Oblique", fontSize=9, textColor=garage_mid, alignment=1)))
